        ...
        <div class="woocommerce-mini-cart-item list-group-item py-3 <?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?>" data-bootscore_product_id="<?php echo esc_attr($product_id); ?>" data-key="<?php echo $cart_item_key; ?>">
          ...
          <div class="row g-3">
            ...
            <div class="item-name col-6">
              ...
              <div class="item-quantity" style="<?php echo $style_qty; ?>">
                <?php echo wc_get_formatted_cart_item_data($cart_item); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                ?>
                <?php if(!$_product->is_sold_individually()) { echo apply_filters('woocommerce_widget_cart_item_quantity', '<span class="quantity">' . sprintf('<span class="qty_text">%s</span> &times; %s', $cart_item['quantity'], $product_price ) . '</span>', $cart_item, $cart_item_key);} // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                ?>
              </div>
            ...
            <div class="remove col-3 text-end">
              ...
              <div class="bootscore-custom-render-total h6 mb-4">
                <?php
                if (get_post_meta($product_id, '_exclusive_points', true) === 'yes') {
                  $points_cost = get_post_meta($product_id, '_points_price', true);
                  echo sprintf(__('%s 🪙', 'woocommerce'), $points_cost);
                } else {
